...
import {
  ...
  ChevronDown, Search, UserCircle2, Image,
} from 'lucide-react'
                ...
                {activeTemplates.length > 0 && (
                  <Card padding="lg">
                    <CardHeader
                      ...
                      subtitle="Select tests to order now"
                    />
                    ...
                    <div ref={testDropdownRef} className="relative">
                      <button
                        ...
                        className="flex w-full items-center justify-between rounded-lg border border-gray-200 bg-white px-3.5 py-2.5 text-sm transition-all hover:border-gray-300 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20"
                      >
                        <span className={selectedTests.length === 0 ? 'text-gray-400' : 'font-medium text-gray-800'}>
                      ...
                      {testDropdownOpen && (
                        <div className="absolute left-0 right-0 top-full z-30 mt-1 overflow-hidden rounded-lg border border-gray-200 bg-white shadow-lg">
                          <div className="border-b border-gray-100 p-2">
                            <div className="relative">
                              ...
                              <input
                                ...
                                className="w-full rounded-md border border-gray-200 py-2 pl-8 pr-3 text-sm outline-none focus:border-blue-400"
                              />
                            </div>
                          </div>
                            ...
                            {filteredTemplatesForDropdown.length === 0 ? (
                              ...
                              return (
                                <button
                                  ...
                                  className={`flex w-full items-center gap-3 px-3 py-2.5 text-left transition-colors hover:bg-gray-50 ${isSelected ? 'bg-blue-50' : ''}`}
                                >
                                  ...
                                  <div className="min-w-0 flex-1">
                                    <p className={`truncate text-sm ${isSelected ? 'font-medium text-blue-800' : 'text-gray-700'}`}>{tmpl.name}</p>
                                    <p className="text-xs text-gray-400 font-mono">{tmpl.code}</p>
                                  </div>
                          ...
                          <div className="border-t border-gray-100 bg-gray-50 px-3 py-2 text-right">
                            <button type="button" onClick={() => { setTestDropdownOpen(false); setTestSearch('') }}
                          ...
                          return (
                            <div key={sel.templateId}
                              ...
                              <div className="flex min-w-0 items-center gap-2">
                                <FlaskConical className="h-3.5 w-3.5 shrink-0 text-blue-500" />
                                <span className="truncate text-sm text-gray-700">{tmpl.name}</span>
                              </div>
                              <div className="flex shrink-0 items-center gap-2">
                                <span className="text-sm font-medium text-gray-800">
                                  {price > 0 ? `₹${price.toLocaleString()}` : '—'}
                                </span>
                                <button type="button" onClick={() => toggleTest(sel.templateId)}
