    ...
    <div>
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          {[
            { label: 'Total Branches', value: totalBranches, bg: 'bg-indigo-50', text: 'text-indigo-600', icon: <MapPin className="h-5 w-5" /> },
        ...
        {isLoading ? <PageLoader /> : branches.length === 0 ? (
          ...
          <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div className="border-b border-slate-100 bg-slate-50 px-6 py-4">
              <h3 className="text-sm font-semibold text-slate-700">All Branches ({branches.length})</h3>
            </div>
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100">
                  <th className="px-6 py-3.5 text-left text-xs font-semibold uppercase tracking-wide text-slate-400">Branch Name</th>
                  <th className="px-6 py-3.5 text-left text-xs font-semibold uppercase tracking-wide text-slate-400">Address</th>
              ...
              <tbody className="divide-y divide-slate-50">
                {branches.map((branch) => (
                  <tr key={branch.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-slate-600 max-w-[200px] truncate">{branch.address ?? '—'}</td>
