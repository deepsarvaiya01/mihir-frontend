      ...
      <Modal
        ...
        footer={
          <>
            ...
            {selectedReport && (
              <Button icon={<Printer className="h-4 w-4" />} onClick={() => printReport(selectedReport)}>Print</Button>
            )}
          </>
        }
      >
        {selectedReport && (
          <div>
            <div className="mb-5 grid grid-cols-2 gap-3 rounded-xl bg-slate-50 p-4 text-sm">
              ...
              <div><span className="text-slate-400">Code:</span> <span className="font-mono text-slate-700">{selectedReport.order.patient?.patientCode ?? '—'}</span></div>
